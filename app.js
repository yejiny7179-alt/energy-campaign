...
function renderExampleImg() {
  ...
  const el = document.getElementById('example-ok-link');
  const a  = document.getElementById('example-ok-url');
  if (link) { el.style.display = 'block'; a.href = link; }
...
function renderLB() {
  for (const g of GRADES) {
    const list   = Object.values(scores).filter(s => s.g === g).sort((a,b) => b.cnt - a.cnt);
    ...
    el.innerHTML = list.map((s,i) => {
      ...
      return `<div class="lb-row">
        ...
        <div class="lb-info">
          <div class="lb-name">
            <span>${s.g}학년 ${s.c}반${isTop?' <span class="lb-reward">추가 상품</span>':''}</span>
            <span class="lb-pt">${s.cnt}회</span>
          </div>
          <div class="lb-bar-wrap"><div class="lb-bar" style="width:${bp}%"></div></div>
          <div class="lb-cnt">${s.cnt}회 참여${qual?' <span style="font-size:10px;background:#E1F5EE;color:#085041;padding:1px 6px;border-radius:20px;font-weight:700;margin-left:4px">🎁 상품 대상</span>':''}</div>
        </div>
      </div>`;
    }).join('');
  }
...
function checkDupWarn() {
  ...
  warn.style.display = (selG && selC && todayMap[`${selG}-${selC}`]) ? 'block' : 'none';
}
...
async function renderAdm() {
  ...
  const snap = await getDocs(query(collection(db,'posts'), where('status','==',admCurTab), orderBy('createdAt','desc')));
  ...
  el.innerHTML = '<div style="background:#fff">' + items.map(p => {
    ...
    const dt = p.createdAt?.toDate ? p.createdAt.toDate().toLocaleString('ko-KR',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}) : '';
    let acts = p.status==='pending'
...
window.admAct = async (id, action) => {
  ...
  try {
    ...
    showToast(action==='approved'?'✅ 승인 완료.':action==='rejected'?'반려 처리되었습니다.':'삭제되었습니다.', action==='approved'?'ok':'');
  } catch(e) { showToast('오류가 발생했습니다.','err'); }
  showLoading(false);
};
