...
async function renderAdm() {
  ...
  if (snap.empty) {
    el.innerHTML = '<div class="empty">해당 상태의 게시물이 없습니다</div>';
    return;
  }
...
function renderSettings(el) {
  el.innerHTML = `<div class="settings-wrap">
    ...
    <p style="font-size:12px;color:#888;margin-bottom:10px;line-height:1.6">
      구글 포토 공유 링크를 입력하면 학생 화면 예시 사진에 표시됩니다.
    </p>
    <input type="url" id="example-link-input"
    ...
    <div style="border-top:0.5px solid rgba(0,0,0,0.08);padding-top:20px">
      ...
      <button class="abtn" style="width:100%;padding:14px;margin-bottom:8px;text-align:left" onclick="exportCSV()">
        📥 게시물 내보내기 (CSV)
      </button>
      <button class="danger-btn" onclick="if(confirm('정말 모든 게시물을 삭제할까요?'))resetAll()">
        🗑 전체 초기화
      </button>
    </div>
  </div>`;
